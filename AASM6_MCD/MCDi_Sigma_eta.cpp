...
LRESULT MCDi_Sigma_eta(COMPLEXARRAY* const retVal,	// Возвращаемый массив
	...
	LPCCOMPLEXSCALAR    eta_cor)				// 3-й аргумент
{
	...
	ErrC = AASM6::get_Sigma_eta(result, M->real, lambda_cor->real, eta_cor->real, x_2, x_1);
...
FUNCTIONINFO AASM6_Sigma_eta = {
	"AASM6_Sigma_eta",					// Имя по которому функция будет вызываться в Mathcad
	...
	"\n\nФункция для определения поправочного множителя, учитывающего влияние сужающейся кормовой части на коэффициент донного сопротивления фюзеляжа, -", // Описание функции
	(LPCFUNCTION)MCDi_Sigma_eta,			// Ссылка на исполняемый код
