...
LRESULT MCDi_Sigma_chi(COMPLEXARRAY* const retVal,	// Возвращаемый массив
	LPCCOMPLEXSCALAR    chi_0)							// 1-й аргумент	
{
	...
	ErrC = AASM6::get_Sigma_chi(result, chi_0->real, x_1);
...
FUNCTIONINFO AASM6_Sigma_chi = {
	"AASM6_Sigma_chi",					// Имя по которому функция будет вызываться в Mathcad
	"\n1: chi_0  - угол стреловидности по передней кромке несущей поверхности, рад",				// Список входных параметров функции
	"\n\nФункция для определения поправочного множителя, учитывающего влияние угла стреловидности по передней кромке на критическое число Рейнольдса, -", // Описание функции
	(LPCFUNCTION)MCDi_Sigma_chi,			// Ссылка на исполняемый код
