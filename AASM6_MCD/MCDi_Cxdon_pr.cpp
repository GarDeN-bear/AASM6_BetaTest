...
LRESULT MCDi_Cxdon_pr(COMPLEXARRAY* const retVal,	// Возвращаемый массив
	LPCCOMPLEXSCALAR    M)					// 1-й аргумент					
{
	...
	ErrC = AASM6::get_Cxdon_pr(result, M->real, x_1);
...
FUNCTIONINFO AASM6_Cxdon_pr = {
	"AASM6_Cxdon_pr",					// Имя по которому функция будет вызываться в Mathcad
	"\n1: M  - число Маха, -",				// Список входных параметров функции
	"\n\nФункция для определения коэффициента донного сопротивления, создаваемого затупленной задней кромкой профиля несущей поверхности, -", // Описание функции
	(LPCFUNCTION)MCDi_Cxdon_pr,			// Ссылка на исполняемый код
