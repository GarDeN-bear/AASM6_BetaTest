...
LRESULT MCDi_Cxdon_Cil(COMPLEXARRAY* const retVal,	// Возвращаемый массив
	LPCCOMPLEXSCALAR    M,							// 1-й аргумент
	LPCCOMPLEXSCALAR    cc,							// 2-й аргумент
	LPCCOMPLEXSCALAR    Re_f06,						// 3-й аргумент
	LPCCOMPLEXSCALAR    Re_f,						// 4-й аргумент
	LPCCOMPLEXSCALAR    xx_tf,						// 5-й аргумент
	LPCCOMPLEXSCALAR    lambda_f)					// 6-й аргумент	
{
...
FUNCTIONINFO AASM6_Cxdon_Cil = {
	"AASM6_Cxdon_Cil",					// Имя по которому функция будет вызываться в Mathcad
	"\n1: M - число Маха, -; \n2: cc - относительная толщина профиля задней несущей поверхности, -; \n3: Re_f06 - число Рейнольдса для фюзеляжа при скорости, соответствующей М=0.6, -; \n4: Re_f - число Рейнольдса для фюзеляжа при скорости, соответствующей М, -; \n5: xx_tf - относительная координата точки перехода ламинарного пограничного слоя в турбулентный на фюзеляже, -; \n6: lambda_f - удлинение фюзеляжа, -",				// Список входных параметров функции
	"\n\nФункция для определения коэффициента донного сопротивления фюзеляжа с цилиндрической кормовой частью, -", // Описание функции
	(LPCFUNCTION)MCDi_Cxdon_Cil,			// Ссылка на исполняемый код
