...
LRESULT MCDi_DELTA_Mcr0_lambda(COMPLEXARRAY* const retVal,	// Возвращаемый массив
...
FUNCTIONINFO AASM6_DELTA_Mcr0_lambda = {
	"AASM6_DELTA_Mcr0_lambda",					// Имя по которому функция будет вызываться в Mathcad
	"\n1: Mcr_pr0  - критическое число Маха профиля при нулевом коэффициенте нормальной силы консолей, -; \n2: lambda_c - удлинение консолей несущей поверхности, -",				// Список входных параметров функции
	"\n\nФункция для определения поправки к критическому числу Маха, учитывающей конечность размаха несущей поверхности, -", // Описание функции
	(LPCFUNCTION)MCDi_DELTA_Mcr0_lambda,			// Ссылка на исполняемый код
