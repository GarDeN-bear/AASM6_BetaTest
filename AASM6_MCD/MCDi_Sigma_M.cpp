...
LRESULT MCDi_Sigma_M(COMPLEXARRAY* const retVal,	// Возвращаемый массив
	...
	LPCCOMPLEXSCALAR    xx_t)					// 2-й аргумент	
{
	...
	ErrC = AASM6::get_Sigma_M(result, M->real, xx_t->real, x_2, x_1);
...
FUNCTIONINFO AASM6_Sigma_M = {
	"AASM6_Sigma_M",					// Имя по которому функция будет вызываться в Mathcad
	...
	"\n\nФункция для определения поправочного множителя, учитывающего влияние числа Маха на коэффициент трения плоской пластинки, -", // Описание функции
	(LPCFUNCTION)MCDi_Sigma_M,			// Ссылка на исполняемый код
