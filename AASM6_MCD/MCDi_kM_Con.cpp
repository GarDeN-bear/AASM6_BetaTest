...
LRESULT MCDi_kM_Con(COMPLEXARRAY* const retVal,	// Возвращаемый массив
	...
	LPCCOMPLEXSCALAR    lambda_nos)				// 2-й аргумент	
{
	...
	ErrC = AASM6::get_kM_Con(result, M->real, lambda_nos->real, x_2, x_1);
...
FUNCTIONINFO AASM6_kM_Con = {
	"AASM6_kM_Con",					// Имя по которому функция будет вызываться в Mathcad
	"\n1: M  - число Маха, -; \n2: lambda_nos - удлинение носовой части фюзеляжа, -",				// Список входных параметров функции
	"\n\nФункция для определения коэффициента торможения потока, вызванного обтеканием конической носовой части, -", // Описание функции
	(LPCFUNCTION)MCDi_kM_Con,			// Ссылка на исполняемый код
