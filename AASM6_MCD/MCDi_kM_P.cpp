...
LRESULT MCDi_kM_P(COMPLEXARRAY* const retVal,	// Возвращаемый массив
	LPCCOMPLEXSCALAR    M,						// 1-й аргумент
	LPCCOMPLEXSCALAR    x,						// 2-й аргумент
	LPCCOMPLEXSCALAR    b_Ac)					// 3-й аргумент
{
	...
	ErrC = AASM6::get_kM_P(result, M->real, x->real, b_Ac->real, x_2, x_1);
...
FUNCTIONINFO AASM6_kM_P = {
	"AASM6_kM_P",					// Имя по которому функция будет вызываться в Mathcad
	"\n1: M  - число Маха, -; \n2: x - расстояние между несущими поверхностями, м; \n3: b_Ac - средняя аэродинамическая хорда консолей передней несущей поверхности, м",				// Список входных параметров функции
	"\n\nФункция для определения коэффициента торможения потока, вызванного обтеканием передней несущей поверхности, -", // Описание функции
	(LPCFUNCTION)MCDi_kM_P,			// Ссылка на исполняемый код
