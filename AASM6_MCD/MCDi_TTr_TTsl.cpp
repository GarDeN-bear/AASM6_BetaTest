...
LRESULT MCDi_TTr_TTsl(COMPLEXARRAY* const retVal,	// Возвращаемый массив
	LPCCOMPLEXSCALAR    M,							// 1-й аргумент
	LPCCOMPLEXSCALAR    key)						// 2-й аргумент	
{
	...
	ErrC = AASM6::get_TTr_TTsl(result, M->real, key->real, x_1);
...
FUNCTIONINFO AASM6_TTr_TTsl = {
	"AASM6_TTr_TTsl",					// Имя по которому функция будет вызываться в Mathcad
	"\n1: M  - число Маха, -; \n2: key - ключ, определяющий относительную температуру, которую возвращает функция (0: Tsl/Tr, 1: Tr/T, 2: Tsl/T)",				// Список входных параметров функции
	"\n\nФункция для определения относительной температуры восстановления и относительной температуры поверхности гладкой стенки, при которой ламинарный пограничный слой является устойчивым, -", // Описание функции
	(LPCFUNCTION)MCDi_TTr_TTsl,			// Ссылка на исполняемый код
