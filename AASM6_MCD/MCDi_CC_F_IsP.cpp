...
LRESULT MCDi_CC_F_IsP(COMPLEXARRAY* const retVal,	// Возвращаемый массив
	LPCCOMPLEXSCALAR    M,						// 1-й аргумент
	LPCCOMPLEXSCALAR    lambda_c,				// 2-й аргумент
	LPCCOMPLEXSCALAR    chi_0)				// 3-й аргумент
{
	...
	ErrC = AASM6::get_CC_F_IsP(result, M->real, lambda_c->real, chi_0->real, x_2, x_1);
...
FUNCTIONINFO AASM6_CC_F_IsP = {
	"AASM6_CC_F_IsP",					// Имя по которому функция будет вызываться в Mathcad
	"\n1: M  - число Маха, -; \n2: lambda_c - удлинение консолей несущей поверхности, -; \n3: chi_0 - угол стреловидности по передней кромке несущей поверхности, рад",			// Список входных параметров функции
	"\n\nФункция для определения коэффициента пропорциональности подсасывающей силы консолей несущей поверхности, -", // Описание функции
	(LPCFUNCTION)MCDi_CC_F_IsP,		// Ссылка на исполняемый код
