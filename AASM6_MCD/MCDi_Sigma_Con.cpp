...
LRESULT MCDi_Sigma_Con(COMPLEXARRAY* const retVal,	// Возвращаемый массив
	...
	LPCCOMPLEXSCALAR    lambda_nos)					// 2-й аргумент	
{
	...
	ErrC = AASM6::get_Sigma_Con(result, M->real, lambda_nos->real, x_2, x_1);
...
FUNCTIONINFO AASM6_Sigma_Con = {
	"AASM6_Sigma_Con",					// Имя по которому функция будет вызываться в Mathcad
	"\n1: M - число Маха, -; \n2: lambda_nos - удлинение носовой части фюзеляжа, -",				// Список входных параметров функции
	"\n\nФункция для определения поправочного множителя, учитывающего отличие коэффициента трения конуса от коэффициента трения плоской пластинки, -", // Описание функции
	(LPCFUNCTION)MCDi_Sigma_Con,			// Ссылка на исполняемый код
