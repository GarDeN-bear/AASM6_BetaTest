...
LRESULT MCDi_xi_D(COMPLEXARRAY* const retVal,	// Возвращаемый массив
	LPCCOMPLEXSCALAR   DD)						// 1-й аргумент					
{
	...
	ErrC = AASM6::get_xi_D(result, DD->real, x_1);
...
FUNCTIONINFO AASM6_xi_D = {
	"AASM6_xi_D",					// Имя по которому функция будет вызываться в Mathcad
	"\n1: DD  - относительный диаметр фюзеляжа в районе несущей поверхности, -",				// Список входных параметров функции
	"\n\nФункция для определения множителя, учитывающего влияние относительного диаметра фюзеляжа на координату приложения дополнительной нормальной силы консолей, -", // Описание функции
	(LPCFUNCTION)MCDi_xi_D,			// Ссылка на исполняемый код
