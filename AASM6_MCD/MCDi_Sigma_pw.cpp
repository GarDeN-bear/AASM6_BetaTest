...
LRESULT MCDi_Sigma_pw(COMPLEXARRAY* const retVal,	// Возвращаемый массив
...
FUNCTIONINFO AASM6_Sigma_pw = {
	"AASM6_Sigma_pw",					// Имя по которому функция будет вызываться в Mathcad
	"\n1: M  - число Маха, -; \n2: chi_c - угол стреловидности по линии максимальных толщин, рад",				// Список входных параметров функции
	"\n\nФункция для определения поправочного множителя, учитывающего степень влияния формы профиля на коэффициент волнового сопротивления несущей поверхности, -", // Описание функции
	(LPCFUNCTION)MCDi_Sigma_pw,			// Ссылка на исполняемый код
