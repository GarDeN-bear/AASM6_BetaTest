...
LRESULT MCDi_Cxcor_Par(COMPLEXARRAY* const retVal,	// Возвращаемый массив
	...
	LPCCOMPLEXSCALAR    eta_cor)				// 3-й аргумент	
{
	...
	ErrC = AASM6::get_Cxcor_Par(result, M->real, lambda_cor->real, eta_cor->real, x_3, x_2, x_1);
...
FUNCTIONINFO AASM6_Cxcor_Par = {
	"AASM6_Cxcor_Par",					// Имя по которому функция будет вызываться в Mathcad
	...
	"\n\nФункция для определения коэффициента сопротивления давления параболической кормовой части, -", // Описание функции
	(LPCFUNCTION)MCDi_Cxcor_Par,		// Ссылка на исполняемый код
