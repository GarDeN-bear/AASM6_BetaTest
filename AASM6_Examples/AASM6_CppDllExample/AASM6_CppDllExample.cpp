// AASM6_CppDllExample.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
#include "..\..\AASM6_Library\AASM6_DYN\AASM6_DYN.h"

// Для подключения динамической библиотеки в свойствах проекта указать путь к библиотеке импорта:
...
// скопировать файл AASM6_DYN.dll в папку с exe файлом этой программы или в другую папку,
// где библиотека может быть найдена

// Добавить в #include файл AASM6_DYN.h
...
void CheckInputComplex(const AASM6_DYN::InputComplex& x, int Number) {
...
int main()
{
    ...
    std::cout << "Пример использования динамической библиотеки аналитической аэродинамики\n";

    AASM6_DYN::ErrorCode* ErrC; //Указатель на структуру для получения кода ошибки, возвращаемого из функции
    AASM6_DYN::InputComplex x_1{ 0.0f, 0.0f, 0.0f }, x_2{ 0.0f, 0.0f, 0.0f }, x_3{ 0.0f, 0.0f, 0.0f }, x_4{ 0.0f, 0.0f, 0.0f }; //Структуры для получения значений и границ входных комплексов, рассчитанных в функции
...
#pragma region get_Cyalf_ConCil (Рис.3.2)
    ...
    float Cyalf_ConCil;
    //Вызов функции
    ErrC = AASM6_DYN::get_Cyalf_ConCil(Cyalf_ConCil, M, lambda_nos, lambda_cil, x_2, x_1);
...
#pragma region get_Cyalf_OgiCil (Рис.3.3)
    ...
    float Cyalf_OgiCil;
    //Вызов функции
    ErrC = AASM6_DYN::get_Cyalf_OgiCil(Cyalf_OgiCil, M, lambda_nos, lambda_cil, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Cyalf_SphCil (Рис.3.4)
    ...
    float Cyalf_SphCil;
    //Вызов функции
    ErrC = AASM6_DYN::get_Cyalf_SphCil(Cyalf_SphCil, M, lambda_nos, lambda_cil, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Cyalf_IsP (Рис.3.5)
    ...
    float Cyalf_IsP;
    //Вызов функции
    ErrC = AASM6_DYN::get_Cyalf_IsP(Cyalf_IsP, M, lambda_c, cc, chi_05, eta_c, x_3, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_kappa_M (Рис.3.13)
    ...
    float kappa_M;
    //Вызов функции
    ErrC = AASM6_DYN::get_kappa_M(kappa_M, M, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Coordinate_zz_v (Рис.3.16)
    ...
    float zz_v;
    //Вызов функции
    ErrC = AASM6_DYN::get_Coordinate_zz_v(zz_v, M, lambda_c, chi_05, zeta_c, x_3, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_interference_v (Рис.3.17)
    ...
    float i_v;
    //Вызов функции
    ErrC = AASM6_DYN::get_interference_v(i_v, zeta_c_II, D_II, l_II, y_v, z_v, x_4, x_3, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_kM_Con (Рис.3.21)
    ...
    float kM_Con;
    //Вызов функции
    ErrC = AASM6_DYN::get_kM_Con(kM_Con, M, lambda_nos, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_kM_P (Рис.3.22)
    ...
    float kM_P;
    //Вызов функции
    ErrC = AASM6_DYN::get_kM_P(kM_P, M, x_I_II, b_Ac_I, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_nn_Eff1 (Рис.3.25)
    ...
    float nn_Eff1;
    //Вызов функции
    ErrC = AASM6_DYN::get_nn_Eff1(nn_Eff1, l_r, l_c, zeta_c, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_nn_Eff2 (Рис.3.28)
    ...
    float nn_Eff2;
    //Вызов функции
    ErrC = AASM6_DYN::get_nn_Eff2(nn_Eff2, M, lambda_c, bb_r, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Cp1_Cp2 (Рис.3.29)
    ...
    float Cp2_Cp1;
    //Вызов функции
    ErrC = AASM6_DYN::get_Cp1_Cp2(Cp2_Cp1, M, 0.0f, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Cx_Cil_N (Рис.3.32)
    ...
    float Cx_Cil_N;
    //Вызов функции
    ErrC = AASM6_DYN::get_Cx_Cil_N(Cx_Cil_N, M, alpha, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_A_IsP (Рис.3.35)
    ...
    float A_IsP;
    //Вызов функции
    ErrC = AASM6_DYN::get_A_IsP(A_IsP, M, zeta_c, Cyalf_IsP, x_3, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Cf_M0 (Рис.4.2)
    ...
    float Cf_M0;
    //Вызов функции
    ErrC = AASM6_DYN::get_Cf_M0(Cf_M0, Re, xx_t, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Sigma_M (Рис.4.3)
    ...
    float Sigma_M;
    //Вызов функции
    ErrC = AASM6_DYN::get_Sigma_M(Sigma_M, M, xx_t, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Re_t0_1 (Рис.4.5)
    ...
    float Re_t0_1;
    //Вызов функции
    ErrC = AASM6_DYN::get_Re_t0_1(Re_t0_1, M, Re, hh, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_TTr_TTsl (Рис.4.8)
    ...
    float TTsl_TTr;
    //Вызов функции
    ErrC = AASM6_DYN::get_TTr_TTsl(TTsl_TTr, M, 0.0f, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Sigma_T (Рис.4.9)
    ...
    float Sigma_T;
    //Вызов функции
    ErrC = AASM6_DYN::get_Sigma_T(Sigma_T, M, TT_s, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Sigma_Con (Рис.4.10)
    ...
    float Sigma_Con;
    //Вызов функции
    ErrC = AASM6_DYN::get_Sigma_Con(Sigma_Con, M, lambda_nos, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Cxnos_Con (Рис.4.11)
    ...
    float Cxnos_Con;
    //Вызов функции
    ErrC = AASM6_DYN::get_Cxnos_Con(Cxnos_Con, M, lambda_nos, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Cxnos_Par (Рис.4.12)
    ...
    float Cxnos_Par;
    //Вызов функции
    ErrC = AASM6_DYN::get_Cxnos_Par(Cxnos_Par, M, lambda_nos, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Cxnos_Ell (Рис.4.13)
    ...
    float Cxnos_Ell;
    //Вызов функции
    ErrC = AASM6_DYN::get_Cxnos_Ell(Cxnos_Ell, M, lambda_nos, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Ms_Con (Рис.4.15)
    ...
    float Ms_Con;
    //Вызов функции
    ErrC = AASM6_DYN::get_Ms_Con(Ms_Con, M, teta_Con, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Cxcor_Con (Рис.4.24a)
    ...
    float Cxcor_Con;
    //Вызов функции
    ErrC = AASM6_DYN::get_Cxcor_Con(Cxcor_Con, M, lambda_cor, eta_cor, x_3, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Cxcor_Par (Рис.4.24б)
    ...
    float Cxcor_Par;
    //Вызов функции
    ErrC = AASM6_DYN::get_Cxcor_Par(Cxcor_Par, M, lambda_cor, eta_cor, x_3, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Cxdon_Cil (Рис.4.26)
    ...
    float Cxdon_Cil;
    //Вызов функции
    ErrC = AASM6_DYN::get_Cxdon_Cil(Cxdon_Cil, M, cc, Re_f06, Re, xx_t, lambda_cil, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Sigma_eta (Рис.4.27)
    ...
    float Sigma_eta;
    //Вызов функции
    ErrC = AASM6_DYN::get_Sigma_eta(Sigma_eta, M, lambda_cor, eta_cor, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Sigma_c (Рис.4.28)
    ...
    float Sigma_c;
    //Вызов функции
    ErrC = AASM6_DYN::get_Sigma_c(Sigma_c, cc, xx_t, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Sigma_chi (Рис.4.29)
    ...
    float Sigma_chi;
    //Вызов функции
    ErrC = AASM6_DYN::get_Sigma_chi(Sigma_chi, chi_0, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Cxw_Rhomb_M1 (Рис.4.30)
    ...
    float Cxw_Rhomb_M1;
    //Вызов функции
    ErrC = AASM6_DYN::get_Cxw_Rhomb_M1(Cxw_Rhomb_M1, M, cc, zeta_c, chi_c, lambda_c, x_4, x_3, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Sigma_pw (Рис.4.32)
    ...
    float Sigma_pw;
    //Вызов функции
    ErrC = AASM6_DYN::get_Sigma_pw(Sigma_pw, M, chi_c, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Mcr_pr (Рис.4.34)
    ...
    float Mcr_pr;
    //Вызов функции
    ErrC = AASM6_DYN::get_Mcr_pr(Mcr_pr, cc, Cn, xx_c, x_3, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_DELTA_Mcr0_lambda (Рис.4.35)
    ...
    //Вызов функции
    ErrC = AASM6_DYN::get_DELTA_Mcr0_lambda(DELTA_Mcr0_lambda, Mcr_pr0, lambda_c, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_DELTA_Mcr0_chi (Рис.4.36)
    ...
    //Вызов функции
    ErrC = AASM6_DYN::get_DELTA_Mcr0_chi(DELTA_Mcr0_chi, Mcr_pr0, chi_c, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Cxdon_pr (Рис.4.38)
    ...
    float Cxdon_pr;
    //Вызов функции
    ErrC = AASM6_DYN::get_Cxdon_pr(Cxdon_pr, M, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Sigma_Cxinos (Рис.4.40)
    ...
    float Sigma_Cxinos;
    //Вызов функции
    ErrC = AASM6_DYN::get_Sigma_Cxinos(Sigma_Cxinos, M, lambda_nos, nos_typ, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_CC_F_IsP (Рис.4.42)
    ...
    float CC_F_IsP;
    //Вызов функции
    ErrC = AASM6_DYN::get_CC_F_IsP(CC_F_IsP, M, lambda_c, chi_0, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Sigma_CF (Рис.4.43)
    ...
    float Sigma_CF;
    //Вызов функции
    ErrC = AASM6_DYN::get_Sigma_CF(Sigma_CF, M, chi_0, alpha, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_ksi_M (Рис.5.7)
    ...
    float ksi_M;
    //Вызов функции
    ErrC = AASM6_DYN::get_xi_M(ksi_M, M, lambda_nos, lambda_cil, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_Coordinate_xxF_IsP (Рис.5.8)
    ...
    float xxF_IsP;
    //Вызов функции
    ErrC = AASM6_DYN::get_Coordinate_xxF_IsP(xxF_IsP, M, lambda_c, chi_05, zeta_c, x_3, x_2, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_ksi_D (Рис.5.11)
    ...
    float ksi_D;
    //Вызов функции
    ErrC = AASM6_DYN::get_xi_D(ksi_D, DD, x_1);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
...
#pragma region get_S_bok
    ...
    //Вызов функции
    ErrC = AASM6_DYN::get_S_bok(S_bok, D, S_nos, lambda_cil, lambda_cor, eta_cor, Ms, b_bs, L_hvs);
    //Проверка не вернула ли функция ошибку
    if (ErrC->Code == 0) //Нет ошибки
    {
